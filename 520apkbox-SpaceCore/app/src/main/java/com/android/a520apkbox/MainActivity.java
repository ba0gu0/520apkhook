...
public class MainActivity extends Activity {
    ...
    private void AppInit(String HackAppPackageName, String HackAppFileName) throws IOException {
        boolean queryAppResult = FCore.get().isInstalled(HackAppPackageName, 0);
        ...
        if (! queryAppResult) {
            ...
            if (Arrays.asList(AssetsFiles).contains(HackAppFileName)){
                ...
                InstallResult installAppResult = FCore.get().installPackageAsUser(HackAppFilePath, 0);
                if (installAppResult.getSuccess()) Log.d(TAG, String.format("基于ApkFile安装, App: %s 安装成功. %s", HackAppFileName, HackAppPackageName));

            }else {
                InstallResult installAppResult = FCore.get().installPackageAsUser(HackAppPackageName, 0);
                if (installAppResult.getSuccess()) Log.d(TAG, String.format("基于PackageName安装, App: %s 安装成功.", HackAppPackageName));
            }
        }

        FCore.get().launchApk(HackAppPackageName, 0);
        Log.d(TAG, String.format("启动App: %s", HackAppPackageName));
    }
}